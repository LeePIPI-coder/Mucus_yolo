...
def compute_froc_metrics(df, score_col, mask_map, fold):
    ...
    # Collect scored predictions per patient, with patient-local GT matching
    scored_pairs = []  # list of (score, is_tp) across all patients
    total_gt_count = 0
    unique_patients = set()
    ...
    for patient_key, group in patient_groups:
        ...
        if len(gt_boxes) == 0:
            # No GT for this patient — all predictions are FP
        ...
        for _, row in group.iterrows():
            ...
            patient_preds.append((score, bbox))

        # ---- Per-patient matching: sort by score desc, greedy match ----
        patient_preds.sort(key=lambda x: x[0], reverse=True)
        detected = [False] * len(gt_boxes)

        for score, pred_box in patient_preds:
            max_iou = 0
            best_gt_idx = -1
    ...
    if not scored_pairs or total_gt_count == 0:
        return {}, 0.0
...
def main():
    ...
    parser.add_argument("--model_path", default="/workspace/3D_resnet_class/experiments/A/re18_pretrain_neg_1/fold_0/best_model.pth",
                        help="Path to trained best_model.pth")
    parser.add_argument("--backbone", default="resnet18",
                        choices=["resnet18", "resnet34"])
    parser.add_argument("--batch_size", type=int, default=32)
    parser.add_argument("--output_dir", default="/workspace/3D_resnet_class/eval_results/re18_pretrain_neg_1—260521")
