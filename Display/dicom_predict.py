...
def main():
    """
    1.命令行参数解析:接收用户输入的各种参数
    2.输入数据加载: 根据输入类型(DICOM 文件夹或 NIfTI 文件)加载医学影像
    3.补丁生成: 将 3D 医学影像分割成 2D 补丁
    4.模型推理: 使用 YOLO 模型对补丁进行目标检测
    5.结果映射与可视化: 将检测结果映射回原始图像并保存
    Input:
        --input: Dicom 目录或者 NIfTI 文件路径
        --model: YOLO 模型路径
        --out: 输出目录
        --patch: 补丁大小 (默认 128)
        --stride: 步长, 默认 = 补丁大小
        --conf: YOLO 置信度阈值 (默认 0.1)
        --device: 推理设备 ID (默认 0)
        --batch-size: 推理批量大小 (默认 64)
    """
