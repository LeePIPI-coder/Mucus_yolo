...
def process_folds(
    ...
    for fold in range(5):
        ...
        pred_csv = data_dir / f"Prediction_GT_FP_fold_{fold}.csv"
        df = pd.read_csv(pred_csv)

        tp = df[df["prediction_type"] == "TP"].copy()
        ...
        for nii_path, group in pbar:
            ...
            for _, row in group.iterrows():
                ...
                if row["label"] == 1:
                    point_num = gt_point_idx
                    gt_point_idx += 1
                    type_str = "GT"
                else:
                    point_num = fp_point_idx
                    fp_point_idx += 1
                    type_str = "FP"
                ...
                for ji in range(n_patches_for_this):
                    ...
                    if not dry_run:
                        ...
                        fname = f"{row['patient_key']}_{point_num}_{type_str}_j{ji}.npy"
