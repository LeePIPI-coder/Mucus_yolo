...
def calculate_iou_3d(box1: dict, box2: dict, z_spacing: float = 1.0) -> float:
    """
    计算两个3D边界框的IoU
    
    Args:
        box1, box2: 边界框字典，包含 x_center, y_center, width, height, slice_idx
        z_spacing: Z轴间距（默认为1.0）
    
    Returns:
        3D IoU值
    """
    # 计算2D IoU
    iou_2d = calculate_iou_2d(box1, box2)
    
    # 计算Z轴重叠
    z1_min = box1['slice_idx'] - 0.5  # 假设每个切片的厚度为1
    z1_max = box1['slice_idx'] + 0.5
    z2_min = box2['slice_idx'] - 0.5
    z2_max = box2['slice_idx'] + 0.5
    
    inter_z_min = max(z1_min, z2_min)
    inter_z_max = min(z1_max, z2_max)
    inter_z = max(0, inter_z_max - inter_z_min)
    
    # 计算3D交集和并集
    inter_3d = iou_2d * inter_z * z_spacing
    
    # 计算3D并集
    area1_3d = ((box1['width'] * 128) * (box1['height'] * 128)) * 1.0 * z_spacing
    area2_3d = ((box2['width'] * 128) * (box2['height'] * 128)) * 1.0 * z_spacing
    union_3d = area1_3d + area2_3d - inter_3d
...
def merge_2d_to_3d_boxes(boxes: List[dict], iou_threshold_2d: float = 0.2, max_slice_gap: int = 1) -> List[dict]:
    """
    将2D检测框合并为3D检测框
    
    Args:
        boxes: 2D边界框列表，每个框包含 x_center, y_center, width, height, slice_idx, confidence
        iou_threshold_2d: 2D IoU阈值，用于判断相邻切片上的框是否属于同一3D目标
        max_slice_gap: 最大切片间隔，允许合并的最大切片距离
    ...
    # 按切片索引排序
    sorted_boxes = sorted(boxes, key=lambda x: x['slice_idx'])
    ...
    for box in sorted_boxes:
        if not current_group:
            ...
            slice_gap = box['slice_idx'] - last_box['slice_idx']
            
            if slice_gap <= max_slice_gap:
                # 计算2D IoU
                iou_2d = calculate_iou_2d(box, last_box)
                if iou_2d >= iou_threshold_2d:
                    # 属于同一3D目标，添加到当前组
                    current_group.append(box)
                else:
                    # 不是同一3D目标，合并当前组并开始新组
                    if current_group:
                        merged_box = merge_group_to_3d(current_group)
...
def merge_group_to_3d(box_group: List[dict]) -> dict:
    """
    将一组2D框合并为一个3D框
    
    Args:
        box_group: 同一3D目标的2D框列表
    ...
    # 计算3D框的属性
    slice_indices = [box['slice_idx'] for box in box_group]
    min_slice = min(slice_indices)
    max_slice = max(slice_indices)
    
    # 计算中心点和尺寸的平均值
    x_centers = [box['x_center'] for box in box_group]
    y_centers = [box['y_center'] for box in box_group]
    widths = [box['width'] for box in box_group]
    heights = [box['height'] for box in box_group]
    
    # 对于预测框，使用最高置信度
    if 'confidence' in box_group[0]:
        confidences = [box['confidence'] for box in box_group]
        max_conf_idx = confidences.index(max(confidences))
        confidence = box_group[max_conf_idx]['confidence']
    else:
        confidence = 1.0  # 标签框默认置信度为1.0
    
    # 构建3D框
    merged_box = {
        'x_center': sum(x_centers) / len(x_centers),
        'y_center': sum(y_centers) / len(y_centers),
        'width': sum(widths) / len(widths),
        'height': sum(heights) / len(heights),
        'slice_idx_min': min_slice,
        'slice_idx_max': max_slice,
        'slice_count': len(box_group),
...
def calculate_iou_3d_merged(box1: dict, box2: dict, z_spacing: float = 1.0) -> float:
    """
    计算两个合并后的3D边界框的IoU
    
    Args:
        box1, box2: 合并后的3D边界框字典，包含 x_center, y_center, width, height, slice_idx_min, slice_idx_max
        z_spacing: Z轴间距（默认为1.0）
    ...
    # 计算Z轴重叠
    z1_min = box1['slice_idx_min']
    z1_max = box1['slice_idx_max'] + 1  # +1因为slice_idx是包含的
    z2_min = box2['slice_idx_min']
    z2_max = box2['slice_idx_max'] + 1
    ...
    # 计算3D体积
    volume1 = ((box1['width'] * 128) * (box1['height'] * 128)) * (box1['slice_idx_max'] - box1['slice_idx_min'] + 1) * z_spacing
    volume2 = ((box2['width'] * 128) * (box2['height'] * 128)) * (box2['slice_idx_max'] - box2['slice_idx_min'] + 1) * z_spacing
...
def nms_3d(boxes: List[dict], iou_threshold: float = 0.5) -> List[dict]:
    """
    对3D边界框执行非极大值抑制
    
    Args:
        boxes: 边界框列表，每个框包含 x_center, y_center, width, height, slice_idx, confidence
        iou_threshold: IoU阈值
    ...
    # 按置信度降序排序
    sorted_boxes = sorted(boxes, key=lambda x: x.get('confidence', 1.0), reverse=True)
    ...
    while sorted_boxes:
        ...
        for box in sorted_boxes:
            # 根据框的结构选择合适的IoU计算方法
            if 'slice_idx_min' in current_box and 'slice_idx_max' in current_box:
                iou = calculate_iou_3d_merged(current_box, box)
            else:
                iou = calculate_iou_3d(current_box, box)
...
def match_detections_3d(predictions: List[dict], ground_truths: List[dict], 
    ...
    for pred_idx in tqdm(sorted_pred_indices, desc="匹配预测与真实标签进度"):
        ...
        for gt_idx, gt in enumerate(ground_truths):
            # 根据框的结构选择合适的IoU计算方法
            if 'slice_idx_min' in pred and 'slice_idx_max' in pred:
                # 两个都是合并后的3D框
                iou = calculate_iou_3d_merged(pred, gt)
            else:
                # 至少有一个是原始2D框
                # 检查是否在相邻切片上
                slice_distance = abs(pred.get('slice_idx', 0) - gt.get('slice_idx', 0))
...
def process_fold_data(csv_path: str, fold_number: int, model_path: Optional[str] = None, 
                     output_dir: str = "./froc_results-3D", iou_threshold: float = 0.5, batch_size: int = 8):
    """
    ...
    # 将2D检测框合并为3D检测框
    logger.info(f"合并2D检测框为3D前：预测框数量={len(all_predictions)}, 标签框数量={len(all_ground_truths)}")
    
    # 合并预测框为3D
    merged_predictions = merge_2d_to_3d_boxes(all_predictions)
    
    # 合并标签框为3D（为标签框添加默认置信度1.0）
    for gt in all_ground_truths:
        if 'confidence' not in gt:
            gt['confidence'] = 1.0
    merged_ground_truths = merge_2d_to_3d_boxes(all_ground_truths)
    
    logger.info(f"合并2D检测框为3D后：预测框数量={len(merged_predictions)}, 标签框数量={len(merged_ground_truths)}")
    
    # 对合并后的3D检测框应用3D NMS
    nms_predictions = nms_3d(merged_predictions, iou_threshold=0.5)
    nms_ground_truths = nms_3d(merged_ground_truths, iou_threshold=0.5)
    
    logger.info(f"应用3D NMS后：预测框数量={len(nms_predictions)}, 标签框数量={len(nms_ground_truths)}")
    
    # 匹配预测结果和真实标签
    matched_pairs, unmatched_preds, unmatched_gts = match_detections_3d(
        nms_predictions, nms_ground_truths, iou_threshold=iou_threshold
    )
